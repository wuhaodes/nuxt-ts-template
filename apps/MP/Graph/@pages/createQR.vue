  ...
  <div class="edit fl">
    <div class="create_QR_form">
      <el-form :model="QRForm" :rules="rules" label-position="top" label-width="80px" ref="QRForm" size="mini">
        <el-row>
          <el-col :span="12">
            ...
            <el-form-item :rules="{ required: true, message: '请输入模版名称', trigger: 'blur' }" label="模版名称" prop="name">
              <el-input placeholder="请输入模版名称" style="width: 200px" v-model="QRForm.name"/>
            </el-form-item>

            <el-form-item label="所属企业" prop="companyId">
              <el-select @change="changeCompanyHandle" filterable placeholder="请选择所属企业" style="width: 200px;" v-model="QRForm.companyId">
                <el-option
                  :key="item.companyId"
                  :label="item.name"
            ...
            <el-form-item label="二维码颜色" prop="color">
              <el-input style="width: 200px" v-model="QRForm.color"/>
              <el-color-picker class="ranks_icon_Hex_picker" v-model="QRForm.color"/>
            </el-form-item>
          ...
          <el-col :span="12">
            ...
            <el-form-item label="二维码类型" prop="ruleId">
              <el-select filterable placeholder="请选择二维码类型" style="width: 200px;" v-model="QRForm.ruleId">
                <el-option
                  :key="item.ruleId"
            ...
            <el-form-item :rules="{ required: true, message: '请选择所属学校', trigger: 'change' }" label="所属学校" prop="schoolId">
              <el-select clearable filterable placeholder="请选择所属学校" style="width: 200px;" v-model="QRForm.schoolId">
                <el-option
                  :key="item.schoolId"
                  :label="item.name"
                  :value="item.schoolId"
              ...
              <el-row :gutter="24">
                <el-col :span="10" style="margin-bottom: 0">
                  <el-upload
                    ...
                    <p class="plus_title" style="padding-left: 0">注：jpg/png格式</p>
                  </el-upload>
                </el-col>
        ...
        <el-form-item label="定制TAG" prop="name">
          <el-input placeholder="请填写定制TAG" style="width: 200px" v-model="QRForm.tag"/>
          <p class="plus_title" style="width: 360px">注：定制TAG为活动码专属扩展信息，请勿随意填写</p>
        </el-form-item>
      </el-form>
    </div>
    <div class="createBtn">
      <el-button :loading="true" size="small" type="primary" v-if="loading">生成</el-button>
      <el-button @click="createQR" size="small" type="primary" v-else>生成</el-button>
    </div>
  </div>

  <div class="show">
    <div class="show_QR">
      <q-r-code-create :color="QRForm.color" :logo-info="{path:QRForm.logo,width:logoWidth*100}" :qr-width="qrWidth*10" :style="`background-color: ${QRForm.background}`"/>
    </div>
  </div>
...
export default Vue.extend({
  ...
  methods: {
    ...
    async createQR () {
      ...
      if (!valid) return
      // this.QRForm.color = this.dark2hex
      // this.QRForm.background = this.light2hex
      // console.log('生成二维码', this.QRForm)
      this.QRForm.qrs[0][3] = this.QRForm.color
      this.QRForm.qrs[0][4] = this.QRForm.logo
      delete this.QRForm.dpi
      this.loading = true
      const result = await uCgi.GraphPrepareSimple(this.QRForm)

      if (result.code === 200) {
        this.$notify({ title: '成功', message: '已成功新增简洁版模版', type: 'success' })
        this.$router.replace({ path: '/mp/graph' })
      }
      setTimeout(() => {this.loading = false}, 3000)
    },

    //添加logo 文件 ElUploadInternalFileDetail
