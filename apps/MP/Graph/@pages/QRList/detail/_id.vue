...
<div class="create_QR_detail">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="二维码颜色" prop="color">
              <el-input style="width: 180px" v-model="QRForm.color"></el-input>
  ...
  <div class="show">
    ...
    <div class="createBtnQR">
      <el-button @click="downloadTxt" size="small" type="primary">查看二维码TXT文件</el-button>
    </div>
    <div class="createBtnQR">
      <el-button @click="downloadZip" icon="el-icon-download" size="small" type="primary" v-if="isDownloadZip">下载二维码图片</el-button>
      <el-button disabled icon="el-icon-loading" size="small" type="primary" v-if="!isDownloadZip">
        下载中...
      </el-button>
    </div>
  </div>
...
export default Vue.extend({
  ...
  async asyncData (query) {
    ...
    const result = await uCgi.CodeDetail({ codeId: query.params.id })
    const QRForm = result.body || {}
    const color = result.body.color || ''
  ...
  methods: {
    ...
    async createQR () {
      ...
      const result = await uCgi.CodeCreate(this.QRForm)
      if (result.code === 200) {
        this.$notify({ title: '成功', message: '已新增二维码', type: 'success' })
        this.$router.replace({ path: '/mp/graph/q_r_list' })
      }
    },

    //downloadZip-下载
    downloadZip () {
      // console.log('ZIp',this.QRForm.zipFile)
      this.isDownloadZip = false
      window.location.href = `${this.OssImg}${this.QRForm.zipFile}`
    ...
    //downloadTXT-下载
    downloadTxt () {
      window.open(`${this.OssImg}${this.QRForm.txtFile}`)
    },
