        ...
        <el-row>
          ...
          <el-col :span="12">
            ...
            <el-form-item :rules="{ required: true, message: '请选择所属学校', trigger: 'change' }" label="所属学校" prop="schoolId">
              <el-select filterable placeholder="请选择所属学校" style="width: 200px;" v-model="QRForm.schoolId">
                <el-option
                  :key="item.schoolId"
                  :label="item.name"
                  :value="item.schoolId"
...
export default Vue.extend({
  ...
  data () {
    return {
      canUse: {
        detailQr: uPassport.checkRole('map.master,map.hr,map.owner,map.market,map.market.design'),
  ...
  async asyncData (query) {
    ...
    const QRForm = {
      ...
      companyId: resultBody.companyId || '',
      schoolId: resultBody.schoolId || '',
      ruleId: resultBody.ruleId || '',
      remark: resultBody.remark || '',
      dpi: resultBody.dpi,
      qrs: resultBody.qrs || [[0, 0, 1000, '', '', 250]],
      // this.QRForm.qrs[0][3] = this.QRForm.color
      // this.QRForm.qrs[0][4] = this.QRForm.logo
      // source: resultBody.source,
      color: resultBody.qrs[0][3] || '',
      logo: resultBody.qrs[0][4] || '',
      weight: resultBody.weight || '',
    }
    const currentCompanyId = resultBody.companyId || ''

    const companySchoolList = schoolList.filter((v: Dic<any>) => (v.companyId === currentCompanyId)) || []
  ...
  methods: {
    ...
    async createQR () {
      ...
      this.QRForm.qrs[0][3] = this.QRForm.color
      this.QRForm.qrs[0][4] = this.QRForm.logo
      this.QRForm.graphId = this.graphId
      ...
      if (result.code === 200) {
        this.$notify({ title: '成功', message: '已成功修改简洁版模版', type: 'success' })
    ...
    handleAvatarSuccessLogo (response: any, file: any) {
      ...
      // console.log('this.QRForm.logo.file', this.QRForm.logo)
    ...
    //删除模版
    graphDelete () {
      this.$confirm('此操作将永久删除简洁模版, 是否继续?', '提示', {
